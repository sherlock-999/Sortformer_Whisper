...
class DiarizationConfig:
    ...
    # Added
    #############
    output_diarisation_mask_dir: Optional[str] = None
    #############
...
class SortformerDiarizer:
    """Minimal wrapper for Sortformer diarization that returns masks in memory."""
    
    def __init__(self, cfg: DiarizationConfig):
        """Initialize with diarization config."""
        self.cfg = cfg
    
    def get_masks(self) -> Dict[str, torch.Tensor]:
        """
        Run diarization and return speaker masks in memory.
        
        Returns:
            Dict mapping audio_name -> speaker_mask (num_speakers, num_frames)
        """
        # Setup (same as main())
        for key in self.cfg:
            self.cfg[key] = None if self.cfg[key] == 'None' else self.cfg[key]
        
        if is_dataclass(self.cfg):
            self.cfg = OmegaConf.structured(self.cfg)
        
        if self.cfg.random_seed:
            pl.seed_everything(self.cfg.random_seed)
        
        if self.cfg.model_path is None:
            raise ValueError("cfg.model_path cannot be None")
        
        # Setup GPU
        torch.set_float32_matmul_precision(self.cfg.matmul_precision)
        if self.cfg.cuda is None:
            device = [0] if torch.cuda.is_available() else 1
            accelerator = 'gpu' if torch.cuda.is_available() else 'cpu'
            map_location = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')
        else:
            device = [self.cfg.cuda]
            accelerator = 'gpu'
            map_location = torch.device(f'cuda:{self.cfg.cuda}')
        
        # Load model
        if self.cfg.model_path.endswith(".nemo"):
            diar_model = SortformerEncLabelModel.restore_from(restore_path=self.cfg.model_path, map_location=map_location)
        else:
            raise ValueError("Model must be .nemo file")
        
        diar_model._cfg.test_ds.session_len_sec = self.cfg.session_len_sec
        trainer = pl.Trainer(devices=device, accelerator=accelerator, precision=self.cfg.precision)
        diar_model.set_trainer(trainer)
        diar_model = diar_model.eval()
        
        # Setup manifest
        diar_model._cfg.test_ds.manifest_filepath = self.cfg.dataset_manifest
        infer_audio_rttm_dict = audio_rttm_map(self.cfg.dataset_manifest)
        
        diar_model._cfg.test_ds.batch_size = self.cfg.batch_size
        diar_model._cfg.test_ds.pin_memory = False
        
        OmegaConf.set_struct(diar_model._cfg, False)
        diar_model._cfg.test_ds.use_lhotse = self.cfg.use_lhotse
        diar_model._cfg.test_ds.use_bucketing = False
        diar_model._cfg.test_ds.drop_last = False
        diar_model._cfg.test_ds.batch_duration = self.cfg.batch_duration
        OmegaConf.set_struct(diar_model._cfg, True)
        
        diar_model._cfg.test_ds.num_workers = self.cfg.num_workers
        diar_model.setup_test_data(test_data_config=diar_model._cfg.test_ds)
        
        # Streaming setup
        if diar_model.streaming_mode:
            diar_model.async_streaming = self.cfg.async_streaming
        ...
        postprocessing_cfg = load_postprocessing_from_yaml(self.cfg.postprocessing_yaml)
        
        # Convert to segments
        all_audiofiles_diarisation_output, _, _ = convert_pred_mat_to_segments(
            infer_audio_rttm_dict,
            postprocessing_cfg=postprocessing_cfg,
            batch_preds_list=diar_model_preds_total_list,
            unit_10ms_frame_count=8,
            bypass_postprocessing=self.cfg.bypass_postprocessing,
            out_rttm_dir=None,
        )
        
        # Convert to masks (no disk save)
        masks = {}
        for audio_file, diarization_output in all_audiofiles_diarisation_output:
            per_speaker_samples = []
            for speaker in diarization_output.labels():
                per_speaker_samples.append(diarization_output.label_timeline(speaker))
            
            # Create mask
            audio_length = round(diarization_output.get_timeline().extent().end * 50)
            diarization_mask = torch.zeros(len(per_speaker_samples), audio_length)
            for spk_idx, speaker_samples in enumerate(per_speaker_samples):
                for start, end in speaker_samples:
                    start_frame = max(0, round(start * 50))
                    end_frame = min(audio_length, round(end * 50))
                    diarization_mask[spk_idx, start_frame:end_frame] = 1
            
            audio_basename = os.path.basename(audio_file).replace(".wav", "")
            masks[audio_basename] = diarization_mask
            print(f"Mask for {audio_basename}: shape {diarization_mask.shape}")
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Sortformer Diarization")
    parser.add_argument("--model_path", required=True, help="Path to .nemo model")
    parser.add_argument("--dataset_manifest", required=True, help="Path to manifest JSON")
    ...
    if args.output_dir:
        ...
        for audio_name, mask in masks.items():
            mask_path = os.path.join(args.output_dir, f"{audio_name}_mask.pt")
            torch.save(mask, mask_path)
        print(f"Saved {len(masks)} masks to {args.output_dir}")